...
export function ToolBar({ title, subtitle, children }: { title: string; subtitle: string; children?: ReactNode }) {
  return (
    <div className="noprint" style={{ position: "sticky", top: 0, zIndex: 60, display: "flex", alignItems: "stretch", height: 74, background: "#15212d", color: "#f4f3f0", borderBottom: `3px solid ${ACCENT}`, boxShadow: "0 4px 16px rgba(14, 24, 32, 0.22)" }}>
      ...
      <Link href="/dashboard" className="tbar-link" style={{ display: "flex", alignItems: "center", padding: "0 18px", borderRight: "1px solid rgba(255,255,255,0.08)", fontFamily: "'Barlow Condensed'", fontWeight: 700, fontSize: 12, letterSpacing: "0.08em", color: "#9aa6b2", flex: "none", textDecoration: "none" }}>← DASHBOARD</Link>
...
export function ToolBarLink({ href, label }: { href: string; label: string }) {
  return (
    <Link href={href} className="tbar-link" style={{ display: "flex", alignItems: "center", padding: "0 18px", borderLeft: "1px solid rgba(255,255,255,0.08)", fontFamily: "'Barlow Condensed'", fontWeight: 700, fontSize: 12, letterSpacing: "0.08em", color: "#9aa6b2", textDecoration: "none" }}>
      {label}
    </Link>
  );
}
...
export function ToolBarButton({ label, onClick }: { label: string; onClick: () => void }) {
  return (
    <button onClick={onClick} className="tbtn" style={{ border: "none", background: ACCENT, color: "#15212d", fontFamily: "'Barlow Condensed'", fontWeight: 700, fontSize: 13, letterSpacing: "0.08em", padding: "0 22px", cursor: "pointer" }}>
      {label}
    </button>
  );
}
