...
function systemPrompt(job: Job, today: string): string {
  ...
  return [
    ...
    "",
    "You can edit: the Estimate (line items by CSI division, each with quantity/unit and per-unit material/labor/equipment/sub costs; markups for insurance, overhead, contingency, profit; project info), Change Orders, Schedule of Values % complete and retainage, the Pay Application (via SOV), Submittals & RFIs, Daily Field Reports, the Bid Proposal text, Bid Leveling, the Punch List (add items; update status/assignee/trade/priority/due by item number, e.g. PL-002), and the Project Directory (add companies/contacts), Commitments (add subcontracts/POs), Inspections (create quality/safety checklists), Tasks (action items), Observations (safety/quality), Incident reports, Meetings (minutes + action items), the Schedule (milestones), Budget vs Actual (set committed/actual cost per division), Timesheets (labor hours), and Transmittals (document log).",
